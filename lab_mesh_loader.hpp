...
namespace lab{
	...
	struct VertexFormat{
		...
		float newpos_x, newpos_y, newpos_z;						//pozitia vertecsilor starii urmatoare
		float newnormal_x, newnormal_y, newnormal_z;			//normalele pentru starea urmatoare
	...
	// fiecare vector contine vertecsi ce au si pozitiile vertecsilor starii urmatoare
	std::vector<VertexFormat> sleep_face;
	std::vector<VertexFormat> surprise_face;
	std::vector<VertexFormat> annoyed_face;
	...
	void initialize(const std::string &filename) {
		...
		if(count == 0) {
			_loadObjFile(filename, sleep_face, indices);
			count++;
		}
		else if(count == 1) {
			_loadObjFile(filename, surprise_face, indices);
			count++;
		}
		else if(count == 2){
			_loadObjFile(filename, annoyed_face, indices);
			count = 0;
		}
	}
	...
	void loadObj(const std::string &filename, unsigned int &vao, unsigned int& vbo, unsigned int &ibo, unsigned int &num_indices){
		...
		// copiaza pozitiile vertecsilor si normalele pentru urmatoare stare
		if(count == 0) {
			for(int i = 0; i < surprise_face.size(); i++) {
				sleep_face[i].newnormal_x = surprise_face[i].normal_x;
				sleep_face[i].newnormal_y = surprise_face[i].normal_y;
				sleep_face[i].newnormal_z = surprise_face[i].normal_z;
				sleep_face[i].newpos_x = surprise_face[i].position_x;
				sleep_face[i].newpos_y = surprise_face[i].position_y;
				sleep_face[i].newpos_z = surprise_face[i].position_z;
			}
		}

		if(count == 1) {
			for(int i = 0; i < annoyed_face.size(); i++) {
				surprise_face[i].newnormal_x = annoyed_face[i].normal_x;
				surprise_face[i].newnormal_y = annoyed_face[i].normal_y;
				surprise_face[i].newnormal_z = annoyed_face[i].normal_z;
				surprise_face[i].newpos_x = annoyed_face[i].position_x;
				surprise_face[i].newpos_y = annoyed_face[i].position_y;
				surprise_face[i].newpos_z = annoyed_face[i].position_z;
			}
		}

		if(count == 2) {
			for(int i = 0; i < sleep_face.size(); i++) {
				annoyed_face[i].newnormal_x = sleep_face[i].normal_x;
				annoyed_face[i].newnormal_y = sleep_face[i].normal_y;
				annoyed_face[i].newnormal_z = sleep_face[i].normal_z;
				annoyed_face[i].newpos_x = sleep_face[i].position_x;
				annoyed_face[i].newpos_y = sleep_face[i].position_y;
				annoyed_face[i].newpos_z = sleep_face[i].position_z;
			}
		}
		...
		if(count == 0) {
			glBufferData(GL_ARRAY_BUFFER, sleep_face.size()*sizeof(VertexFormat), &sleep_face[0], GL_STATIC_DRAW);
			count++;
		}
		else if(count == 1) {
			glBufferData(GL_ARRAY_BUFFER, surprise_face.size()*sizeof(VertexFormat), &surprise_face[0], GL_STATIC_DRAW);
			count++;
		}
		else if(count == 2) {
			glBufferData(GL_ARRAY_BUFFER, annoyed_face.size()*sizeof(VertexFormat), &annoyed_face[0], GL_STATIC_DRAW);
			count = 0;
		}
		...
		glVertexAttribPointer(3,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(sizeof(float)*8));		//trimite pozitii vertecsi stare urmatoare pe pipe 3
		glEnableVertexAttribArray(4);
		glVertexAttribPointer(4,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(sizeof(float)*11));	//trimite normale stare urmatoare pe pipe 4
